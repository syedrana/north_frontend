...
function normalizeSection(section = {}) {
  ...
  const flashSale = data?.flashSale && typeof data.flashSale === "object" ? data.flashSale : null;
  ...
  return {
    ...
    flashSale,
    items:
      normalizedType === "flash_sale" && flashSale
        ? ensureArray(flashSale?.products)
        : sectionItems.length > 0
          ...
          : dataItems.length > 0
            ...
            : categories.length > 0
              ? categories
              : products,
    endsAt: section?.endsAt || data?.endsAt || flashSale?.endTime || "",
    startsAt: section?.startsAt || data?.startsAt || flashSale?.startTime || "",
  };
}
